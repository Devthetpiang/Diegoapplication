...
public class DiegoClient {
    ...
    private static DiegoApiInterface sDiegoService;

    //returns the Singleton Instance of our Rest client or Creates an new instance
    public static DiegoApiInterface getDiegoApiClient(Context ctx) {
        if (sDiegoService == null) {
            ...
            try {
                adapter = new RestAdapter.Builder().setEndpoint(ctx.getString(R.string.api_endpoint_diego))
                        .setClient(client)
            ...
            sDiegoService = adapter.create(DiegoApiInterface.class);
        }

        return sDiegoService;
    }
}
